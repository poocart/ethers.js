...
export function RNdecrypt(json: string, password: Arrayish, options?: any, progressCallback?: ProgressCallback): Promise<SigningKey> {
    ...
    return new Promise(function(resolve, reject) {
        ...
        if (kdf && typeof(kdf) === 'string') {
            if (kdf.toLowerCase() === 'scrypt') {
                ...
                // @ts-ignore
                var RNsalt = Object.keys(salt).map(key => salt[key]).filter(v => Number.isInteger(v));
...
export function RNencrypt(privateKey: Arrayish | SigningKey, password: Arrayish | string, options?: EncryptOptions, progressCallback?: ProgressCallback): Promise<string> {
    ...
    return new Promise(function(resolve, reject) {
        ...
        //   - 32 bytes   AES key to encrypt mnemonic with (required here to be Ethers Wallet)
        // @ts-ignore
        const RNsalt = Object.keys(salt).map(key => salt[key]).filter(v => Number.isInteger(v));
        const result = RNscrypt(password, RNsalt, N, r, p, 64);
        result.then((key: any) => {
            if (key) {
                key = looseArrayify(key);
